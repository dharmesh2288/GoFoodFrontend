...
export default function Home() {
    ...
    const loadData = async () => {
        let response = await fetch("https://gofoodbackend-hf66.onrender.com/api/foodData", {
            method: "POST",
            headers: {
                    ...
                    <div className="carousel-inner" id='carousel'>
                        <div className="carousel-caption" style={{ zIndex: "10" }}>
                            <div className="d-flex justify-content-center">
                                <input className="=div-control me-2" type="search" placeholder="Search" aria-label="Search" value={serch} onChange={(e) => { setSerch(e.target.value) }} />
                    ...
                    foodCat !== []
                        ? foodCat.map((data) => {
                            return (<div className='row mb-3'>
                                ...
                                {foodItem !== []
                                    ?
                                    foodItem.filter((item) => (item.CategoryName === data.CategoryName) && (item.name.toLowerCase().includes(serch.toLocaleLowerCase())))
                                        .map(filterItems => {
                                            return (
